...
interface Lead {
  ...
  situacao: string;
  website?: string;
  parada_origem?: string; // Vincula o lead à cidade da rota
}
...
export default function PlanejadorRotasPage() {
  ...
  const calcularItinerario = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const listaParadas = (dados.planoRotaAI.paradas_sugeridas || []).map((p: any) => ({
        ...p,
        selecionada: true // Começa todo mundo marcado por padrão
      }));
  ...
  // Passo 2: Disparar a mineração em lote no BigQuery para as cidades confirmadas
  const gerarAgendaProspeccao = async () => {
    ...
    try {
      ...
      // Loop cirúrgico disparando chamadas para a nossa API do BigQuery reaproveitando a inteligência
      for (const cidade of cidadesAlvo) {
        // Criamos um prompt simulado focado exatamente no código do município que a IA mapeou
        ...
        if (dados.leads && dados.leads.length > 0) {
          const tratados = dados.leads.map((l: any) => ({
            ...
            parada_origem: cidade.cidade_nome // Guarda qual parada gerou esse lead
          }));
  ...
  // Filtra os leads na tabela conforme o clique nas abas das cidades
      ...
      <div className="max-w-[1700px] mx-auto space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <form onSubmit={calcularItinerario} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div>
              <label className="block font-black text-slate-500 uppercase text-[10px] tracking-widest mb-1.5">Nicho / O que buscar nas paradas?</label>
              <input 
                type="text" value={atividade} onChange={e => setAtividade(e.target.value)}
                placeholder="Ex: Distribuidores de Medicamentos, autopeças..." 
                className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold outline-none focus:border-indigo-500"
              />
            </div>
            <button
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-slate-400 text-[11px]">Marque ou desmarque as cidades onde deseja que o comercial faça paradas para visitas físicas.</p>
              </div>
        ...
        {/* PASSO 3: TABELA DE LEADS SEPARADAS POR ABAS DE CIDADES */}
        {leadsDaRota.length > 0 && (
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            
            {/* ABAS SELETORAS DE CIDADES (O SEGREDO DA ORGANIZAÇÃO DO SDR) */}
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-slate-500 uppercase text-[10px] font-black tracking-widest border-b border-slate-200">
                    ...
                    <th className="p-3.5">Nome Comercial Google</th>
