...
export default function OperacoesSecPage() {
  ...
  const processarArquivoExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setProcessando(true);
    setStatusMsg("Varrendo blocos de datas e processando DRE Securitizadora...");
    ...
    try {
      ...
      let dataBlocoAtiva: string | null = null;
      ...
      for (let i = 0; i < rawRows.length; i++) {
        ...
        if (dataBlocoAtiva && row.length > 10 && row[5]) {
          ...
          const vop = parseValorReal(row[10]);
          const receitaOp = parseValorReal(row[12]) + parseValorReal(row[13]) + parseValorReal(row[14]);

          agrupamento[rawCedente].push({
            data_operacao: dataBlocoAtiva,
            mes_ano: extrairMesAnoDeISO(dataBlocoAtiva),
            vop: vop,
            receita: receitaOp
          });
        }
      ...
      for (const [cedentePlanilha, ops] of Object.entries(agrupamento)) {
        ...
        const totalVop = ops.reduce((acc, o) => acc + o.vop, 0);
        const totalReceita = ops.reduce((acc, o) => acc + o.receita, 0);
      ...
      setStatusMsg("✅ Consolidação em memória concluída.");
    } catch (err: any) {
  ...
  const transferirDadosProSupabase = async () => {
    ...
    setProcessando(true);
    setStatusMsg("🚀 Expurgando duplicidades e transmitindo dados de DRE...");
    ...
    try {
      ...
      alert("❌ Erro ao salvar dados no banco: " + err.message);
    } finally {
      setProcessando(false);
    }
  };
      ...
      {linhasConciliadas.length > 0 && (
        <div className="space-y-4">
          <div className="bg-slate-900 text-white p-4 rounded-xl flex justify-between items-center font-bold">
            <span>Validação cadastral de DRE - Securitizadora</span>
              ...
              <table className="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 font-bold uppercase text-slate-400 text-[10px] tracking-wider h-11">
                    ...
                    <th className="p-4 text-center w-36">Lotes Operados</th>
                      ...
                      <td className="p-4">
                        {linha.status.startsWith("🟢") ? (
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-[10px] font-mono font-bold text-slate-400">CNPJ Vinculado</span>
                          </div>
                        ) : (
