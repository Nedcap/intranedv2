...
export default function NedHubPage() {
      ...
      if (user) {
        ...
        if (dbLeads) {
          setLeads(dbLeads.map((l: any) => ({
            ...
            telefone: l.telefone,
            estagio: l.estagio_id,
            funilId: l.funil_id || "vendas",
  ...
  // 🔌 CHAMADA DA SUA PONTE LOCAL REFINADA DO SQLITE!
  const handleConsultarCnpjReceita = async () => {
    ...
    try {
      ...
      alert(`⚠️ Info: ${err.message}. Digitação manual liberada.`);
    }
  };
  ...
  const handleSalvarNovoLead = async () => {
    ...
    try {
      const payload = {
        cnpj: inputCnpj,
        razao_social: inputRazao,
        nome_contato: inputContato,
        telefone: inputTelefone,
        funil_id: funilAtivo,
        estagio_id: funilAtivo === "vendas" ? "sem_contato" : "visita_agendada",
  ...
  const moverEstagioCard = async (id: string, direcao: "proximo" | "voltar") => {
    ...
    try {
      const { error } = await supabase
        .from("crm_leads")
        .update({ estagio_id: listaEstagios[novoIdx] })
        .eq("id", id);
  ...
  // Separação dinâmica por coluna dependendo do funil selecionado
  ...
  return (
    <div className="h-[calc(100vh-40px)] flex flex-col font-sans text-slate-700 bg-slate-50 text-[11px] overflow-hidden p-4 space-y-4">
      ...
      {/* SELETOR DE FUNIL ESTILO RD STATION */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 pb-3 bg-white p-4 rounded-xl shadow-2xs gap-4">
        <div className="flex items-center gap-4">
          ...
          {/* CONTROL SWITCHER DE FUNIS */}
          <select 
            value={funilAtivo} 
            onChange={(e) => setFunilAtivo(e.target.value as any)}
            className="p-2 border border-blue-300 rounded-lg bg-blue-50 text-blue-900 font-black uppercase text-[10px] outline-none cursor-pointer shadow-2xs"
          >
      ...
      {/* RENDERIZADOR DINÂMICO DE GRIDS (Ajusta automático de 4 ou 5 colunas dependendo do funil) */}
      {!carregando && (
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 bg-slate-900 text-white font-black uppercase text-[10px] flex justify-between items-center">
              ...
              <button onClick={() => setModalNovoLead(false)}>✕</button>
            </div>
            <div className="p-4 space-y-3 overflow-y-auto flex-1">
              <div className="flex gap-2 items-end">
                <div className="flex-1">
                  <label className="block text-[9px] font-black text-slate-400 uppercase">CNPJ da Empresa</label>
                  <input type="text" placeholder="Apenas números" value={inputCnpj} onChange={e => setInputCnpj(e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg font-mono font-bold text-slate-800 bg-amber-50/10" />
                </div>
                <button type="button" onClick={handleConsultarCnpjReceita} className="p-2 bg-amber-500 hover:bg-amber-600 text-white font-black rounded-lg uppercase text-[10px] h-9">⚡ Puxar Bot</button>
              </div>
              <div>
                <label className="block text-[9px] font-black text-slate-400 uppercase">Nome da Negociação / Razão Social</label>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><label className="block text-[9px] font-black text-slate-400 uppercase">Contato</label><input type="text" value={inputContato} onChange={e => setInputContato(e.target.value)} className="w-full p-2 border border-slate-200 rounded-lg" /></div>
                <div><label className="block text-[9px] font-black text-slate-400 uppercase">Telefone</label><input type="text" value={inputTelefone} onChange={e => setInputTelefone(e.target.value)} className="w-full p-2 border border-slate-200 rounded-lg" /></div>
              </div>
              {Object.keys(valoresCamposCustomizados).length > 0 && (
                <div className="bg-blue-50/50 p-3 rounded-lg border border-blue-100 space-y-1">
                  <span className="text-[9px] font-black text-blue-700 uppercase">Metadados Capturados pelo Bot:</span>
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-200 flex justify-end gap-2">
              <button onClick={() => setModalNovoLead(false)} className="px-3 py-1.5 bg-slate-200 font-bold rounded-lg uppercase">Cancelar</button>
              <button onClick={handleSalvarNovoLead} className="px-4 py-1.5 bg-blue-600 text-white font-black rounded-lg uppercase">Salvar Negociação</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="bg-white p-3 border border-slate-200 rounded-xl shadow-2xs hover:shadow-xs transition-all space-y-2 group">
      ...
      {Object.keys(lead.dadosCustomizados).length > 0 && (
        <div className="bg-slate-50 p-1.5 rounded-lg space-y-0.5 text-[8px] font-mono border border-slate-100">
          {Object.entries(lead.dadosCustomizados).slice(0, 2).map(([k, v]: any) => (
            <div key={k} className="truncate"><span className="text-slate-400">{k}:</span> <span className="text-slate-700 font-bold">{v}</span></div>
          ))}
        </div>
      )}
      <div className="flex justify-between items-center pt-1 border-t border-slate-100">
        <button onClick={() => onMover(lead.id, "voltar")} className="px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-md font-bold">◀</button>
        <span className="text-[8px] uppercase tracking-widest text-emerald-600 font-black">🔒 Carteira</span>
        <button onClick={() => onMover(lead.id, "proximo")} className="px-2 py-0.5 bg-slate-900 text-white hover:bg-blue-600 rounded-md font-bold">▶</button>
      </div>
    </div>
  );
}