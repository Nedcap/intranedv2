...
export default function GerenciarHierarquiaPage() {
  ...
  const removerDaArvore = (usuarioId: string) => {
    if (!confirm("Tem certeza que deseja remover este usuário e todos os subordinados dele da hierarquia?")) return;
    try {
      // Remove o vínculo do usuário atual
      const { data, error } = supabase
        .from("usuarios")
        .update({ permissoes: { ...usuariosSistema.find(u => u.id === usuarioId).permissoes, lider_id: null } })
        .eq("id", usuarioId);

      if (error) throw error;

      // Remove recursivamente o vínculo de todos os subordinados dele
      const subordinados = usuariosSistema.filter(u => u.permissoes?.lider_id === usuarioId);
      for (const sub of subordinados) {
        removerDaArvore(sub.id);
      }

      // Recarrega os dados para atualizar a interface
      carregarDadosHierarquia();
    } catch (error: any) {
      alert(`❌ Erro ao remover usuário da hierarquia: ${error.message}`);
    }
  };
  ...
  const RenderizarFilhosOrganograma = ({ liderId }: { liderId: string | null }) => {
    ...
    return (
      <div className="flex justify-center gap-x-12 pt-8 m-auto w-max min-w-full">
        {filhos.map((sub, idx) => {
          const temFilhos = usuariosSistema.some(u => hierarquiaLocal[u.id] === sub.id);
          return (
            <div key={sub.id} className="flex flex-col items-center relative text-center">
              
              {/* CARD EXPANDIDO NA ÁRVORE */}
              <div
                draggable
    ...
    <div className="p-6 space-y-6 font-sans bg-slate-50 min-h-screen text-[12px] flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden w-full h-full items-stretch">
        ...
        >
          ...
          <RenderizarFilhosOrganograma liderId={null} />

          {raizesDaArvore.length === 0 && !carregando && (
