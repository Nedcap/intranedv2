...
export default function ControleComercialVisitasPage() {
  ...
  const buscarCardsComercial = useCallback(async () => {
    ...
    try {
      ...
      // Busca Empresas Aprovadas na tabela de Analises
      const { data: analisesAprovadas, error: analisesError } = await supabase
        .from("analises")
        .select("cnpj")
        .ilike("status_comite", "aprovado");

      if (analisesError) console.warn("Aviso: Não foi possível checar análises aprovadas", analisesError);
      ...
      if (leadsData) {
        ...
        const dadosMapeados: VisitaRow[] = leadsData.map((item: any) => {
          ...
          const temAprovacaoNoComite = analisesAprovadas?.some((a: any) => {
          ...
          if (temAprovacaoNoComite && statusComiteSalvo !== "Aprovado" && statusComiteSalvo !== "Pago") {
            ...
            supabase.from("crm_leads").update(updatePayload).eq("id", item.id).then();
          }
  ...
  const kpisGlobais = useMemo(() => {
    ...
    return { totalAgendamentosPendentes, totalComiteAprovado, totalPago };
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto pb-10 text-[13px] font-sans text-slate-700">
      ...
      {carregando && (
        <div className="p-8 font-bold text-center text-xs tracking-widest uppercase text-slate-500 bg-slate-50 border border-slate-200 rounded-xl animate-pulse shadow-inner">
          ⏳ Carregando fluxo de leads do CRM e verificando aprovações...
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            ...
            <div className="bg-white border border-slate-200 p-5 rounded-xl border-l-4 border-l-blue-600 shadow-xs flex flex-col justify-center">
              ...
              <div className="text-2xl font-black mt-1 font-mono text-blue-800">{formatarMoeda(kpisGlobais.totalAgendamentosPendentes)}</div>
            </div>
            <div className="bg-white border border-slate-200 p-5 rounded-xl border-l-4 border-l-amber-500 shadow-xs flex flex-col justify-center">
              <span className="text-[10px] font-black text-slate-400 block uppercase tracking-wider">Comitê Aprov. (A Receber)</span>
              <div className="text-2xl font-black mt-1 font-mono text-amber-700">{formatarMoeda(kpisGlobais.totalComiteAprovado)}</div>
            </div>
            <div className="bg-emerald-600 text-white p-5 rounded-xl shadow-md flex flex-col justify-center">
              <span className="text-[10px] font-black text-emerald-200 block uppercase tracking-wider">✅ Total Pago (Visitas + Comitê)</span>
              <div className="text-2xl font-black mt-1 font-mono">{formatarMoeda(kpisGlobais.totalPago)}</div>
            </div>
          </div>
                ...
                <table className="w-full text-left border-collapse min-w-[1100px]">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-black uppercase text-[10px] tracking-wider">
                      ...
                      <th className="p-4 w-80">Lead Comercial / Contato</th>
                      ...
                      return (
                        <React.Fragment key={v.id}>
                          <tr className={`hover:bg-slate-50/70 transition-colors ${expandido ? 'bg-slate-50' : ''}`}>
                            ...
                            <td className="p-4">
                              <div className="font-black text-slate-900 truncate max-w-[280px] uppercase" title={v.nome}>{v.nome}</div>
                              <span className="text-[10px] text-slate-400 font-bold truncate max-w-[280px] block font-mono mt-0.5">
                                {v.cnpj ? `CNPJ: ${v.cnpj}` : (v.email || v.telefone || "Sem contatos")}
                              </span>
                            </td>
