      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-xs overflow-hidden">
        <table className="w-full text-left border-collapse text-[13px]">
          ...
          <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
            {socios.length === 0 ? (
              <tr><td colSpan={7} className="p-6 text-center text-slate-400">Nenhum restritivo de sócio ativo.</td></tr>
            ) : (
              socios.map((item, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50">
                  <td className="p-4">
                    <div className="font-bold text-slate-900">{item.nome_socio}</div>
                    <div className="text-slate-400 text-xs font-mono mt-0.5">CNPJ Vínculo: {item.cnpj_empresa || "N/A"}</div>
                  </td>
