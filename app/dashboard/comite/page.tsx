...
export default function ComitePage() {
  ...
  // 🎯 SINCRONIZAÇÃO COMPLETA COM A TABELA OFICIAL ANALISES_CREDITO
  const carregarComite = async () => {
    try {
      setCarregando(true);
      const userStr = localStorage.getItem("intraned_user");
      
      // Aponta para a tabela unificada analises_credito
      let queryComite = supabase.from("analises_credito").select("*");
      let queryEsteira = supabase.from("analises_credito").select("*");

      if (userStr) {
        const user = JSON.parse(userStr);
        const cargoUser = String(user.cargo || user.perfil || "").trim().toLowerCase();

        // 🛡️ Filtro de escopo comercial ativo
        if (cargoUser === "comercial" && user.nome) {
          queryComite = queryComite.eq("comercial_nome", user.nome);
          queryEsteira = queryEsteira.eq("comercial_nome", user.nome);
        }
      }
      
      // Carrega pauta ativa do comitê (Empresas prontas ou finalizadas)
      const { data: dataComite } = await queryComite.in("status", ["em_revisao_humana", "concluido", "aprovado", "reprovado"]).order("created_at", { ascending: false });
      if (dataComite) {
        setAnalises(dataComite);
        for (const item of dataComite) {
          await carregarVotosIniciais(item.razao_social);
          // Prepara preview do Dossiê HTML estruturado se houver link de anexo
          if (item.dados_consolidados?.anexos?.organograma_url) {
             baixarHtmlInline(item.id, item.dados_consolidados.anexos.organograma_url);
          }
        }
      }

      // Carrega esteira operacional (Em processamento pela IA ou pendente de documentos)
      const { data: dataAnalise } = await queryEsteira.in("status", ["em_processamento_ia", "aguardando_docs"]).order("created_at", { ascending: false });
      if (dataAnalise) setEmpresasAnalise(dataAnalise);

    } catch (err) {
      console.error("Erro ao carregar dados do comitê:", err);
    } finally {
      setOriginalCarregando(false);
    }
  };

  const [originalCarregando, setOriginalCarregando] = useState(true);
  ...
  useEffect(() => {
    ...
    const canalChat = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "chat_comite", filter: `empresa_nome=eq.${empresaAlvo.razao_social}` }, (payload) => {
  ...
  // 🔥 FORÇA A GRAVAÇÃO EXECUTIVA DA DECISÃO FINAL DENTRO DO JSON DE DADOS_CONSOLIDADOS
  const forcarDecisaoMaster = async (empresaItem: any, decisaoFinal: "Aprovado" | "Reprovado") => {
    const conf = confirm(`⚠️ DECISÃO EXECUTIVA: Deseja mover a empresa ${empresaItem.razao_social} para ${decisaoFinal}?`);
    if (!conf) return;

    try {
      setOriginalCarregando(true);
      const e = empresaItem.razao_social;

      // Monta e atualiza o parecer de comitê no JSON
  ...
  // 🔥 RECALCULA O PLACAR AO VIVO E INJETA OS VOTOS ATUALIZADOS NA PROPRIEDADE PARECER_COMITE DO DOSSIÊ
  const processarVotoWeb = async (empresaItem: any) => {
    ...
    try {
      ...
      // 1. Registra o voto na tabela de auditoria viva
      ...
      // 2. Busca o histórico de votos consolidados para recalcular o placar
      ...
      // 3. Atualiza o JSON interno injetando o log de votação no parecer_comite
      const dcAtual = empresaItem.dados_consolidados || {};
  ...
  const handleCriarAnalise = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("analises_credito").insert({
        razao_social: nomeNovaEmpresa.trim().toUpperCase(),
        cnpj: "00000000000000",
  ...
  const handleSalvarEdicao = async (item: any) => {
    try {
      ...
      dcAtual.pendencias_texto = pendenciaTexto;

      const { error } = await supabase.from("analises_credito").update({
        dados_consolidados: dcAtual
      }).eq("id", item.id);
      ...
      alert(`❌ Erro: ${err.message}`);
    } finally { setOriginalCarregando(false); }
  };
  ...
  const handleDeletarAnalise = async (id: string) => {
    ...
    try {
      setOriginalCarregando(true);
      const { error } = await supabase.from("analises_credito").delete().eq("id", id);
      if (error) throw error;
      await carregarComite();
    } catch (err: any) { 
      alert(`❌ Erro: ${err.message}`);
  ...
  const activarModoLupaExecutiva = async (empresa: any) => {
    ...
    try {
      const { data } = await supabase
        .from("chat_comite")
        .select("*")
        .eq("empresa_nome", empresa.razao_social)
  ...
  if (modoFocoComite && empresaFocoAtivo) {
    const listaDeVotos = votosAoVivo[empresaFocoAtivo.razao_social] || [];
      ...
      <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col font-sans h-screen w-screen overflow-hidden text-[13px]">
        <div className="bg-slate-950 text-white p-3 px-6 flex justify-between items-center shadow-lg border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-black uppercase text-white tracking-wide">{empresaFocoAtivo.razao_social}</h2>
            ...
            <div className="flex-1 bg-white rounded-xl shadow-2xl overflow-hidden border border-slate-800">
              {htmlPreview ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center bg-slate-950 text-slate-500 italic text-sm gap-2">
                  ...
                  Nenhum relatório HTML compilado anexado a esta conta.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="w-[30%] h-full p-4 flex flex-col space-y-4 bg-slate-950/40">
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl shadow-md flex-1 flex flex-col overflow-hidden text-left">
              ...
              <div className="flex gap-2 mt-2 shrink-0">
                <input type="text" value={novaMsg} onChange={(e) => setNovaMsg(e.target.value)} onKeyDown={(e) => e.key === "Enter" && enviarMensagemChat(empresaFocoAtivo.razao_social)} placeholder="Mensagem..." className="flex-1 p-2 bg-slate-950 text-white border border-slate-800 rounded-lg text-xs outline-none focus:border-blue-500 font-medium" />
                <button onClick={() => enviarMensagemChat(empresaFocoAtivo.razao_social)} className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-3 rounded-lg cursor-pointer transition-all">Mandar</button>
              </div>
            </div>
          </div>
        </div>
  ...
  // 🏛️ RENDEREIZAÇÃO DA VISÃO PADRÃO
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-8 text-[13px]">
      {originalCarregando && <div className="fixed inset-0 bg-white/40 z-50 flex items-center justify-center font-bold text-slate-500">Sincronizando esteira...</div>}
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
              {analises.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50">
                  <td className="p-2.5 font-bold text-slate-900 uppercase">{item.razao_social}</td>
                  <td className="p-2.5 font-mono text-slate-500">{item.cnpj}</td>
                  <td className="p-2.5 text-center text-slate-500 font-mono">{new Date(item.created_at).toLocaleDateString("pt-BR")}</td>
              ...
              {analises.length === 0 && (
                <tr><td colSpan={isMaster ? 6 : 5} className="p-4 text-center text-slate-400 italic">Nenhuma análise em pauta na mesa.</td></tr>
              )}
            </tbody>
          </table>
        </div>
                ...
                empresasAnalise.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50/40">
                      <td className="p-2.5 font-bold text-blue-900 uppercase">{item.razao_social}</td>
                      <td className="p-2.5 font-mono text-slate-500">{item.cnpj}</td>
                      <td className="p-2.5">
