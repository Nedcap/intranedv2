...
const montarHtmlDossie = (item: any) => {
  ...
  const renderizarCabecalhoEmpresas = () => {
    if (analise.empresas_principais && analise.empresas_principais.length > 0) {
      return `
        <div>
          ...
          `).join('')}
          <div class="meta" style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid rgba(255,255,255,0.2);">
            Data Emissão: ${dataAtual} | Analista: ${analise.analista || item.comercial || '-'} | Gerente: ${analise.gerente || '-'}
          </div>
        </div>
      `;
    }
    ...
    return `
      <div>
        <h1 style="font-size: 2rem; margin-bottom: 0.2rem;">${empresaNome}</h1>
  ...
  const refRows = analise.referencias && analise.referencias.length > 0
    ? analise.referencias.map((r: any) => {
        ...
        <tr>
          ...
          <td class="text-right">
             <div style="font-weight:bold; color:var(--red); font-family: monospace;">${formatarMoeda(r.risco_total)}</div>
  ...
  <head>
      ...
      <title>Dossiê Executivo - Comitê</title>
