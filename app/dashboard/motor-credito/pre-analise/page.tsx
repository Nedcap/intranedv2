...
export default function PreAnalisePage() {
  ...
  const carregarFotoDaConsulta = (itemHistorico: any) => {
    ...
    if (itemHistorico.dados_empresa && itemHistorico.dados_financeiros) {
      ...
      setDadosCompliance(itemHistorico.dados_compliance || { sancionado: false, ocorrencias: 0, detalhes: [] });
      setMostrarJsonBruto(false);
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="p-8 space-y-10">
              ...
              <section>
                <h2 className="text-lg font-black text-blue-900 uppercase tracking-wide flex items-center gap-3 border-b-2 border-slate-100 pb-2 mb-6">
                  <span className="w-1.5 h-5 bg-blue-600 rounded-full"></span>
                  4. Compliance e Integridade (Portal da Transparência / CGU)
                </h2>

                {dadosCompliance ? (
                  <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                    <div className={`p-4 flex justify-between items-center border-b ${dadosCompliance.sancionado ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}>
                      <span className="text-xs font-black uppercase tracking-widest text-slate-800">
                        {dadosCompliance.sancionado ? "⚠️ Sanções Governamentais Detectadas" : "✅ Empresa Regular / Sem Sanções Institucionais"}
                      </span>
                      <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase shadow-sm ${dadosCompliance.sancionado ? 'bg-red-600 text-white' : 'bg-emerald-600 text-white'}`}>
                        {dadosCompliance.ocorrencias} Ocorrência(s)
                      </span>
                    </div>

                    {dadosCompliance.sancionado ? (
                      <table className="w-full text-left border-collapse">
                        <thead className="bg-white border-b border-slate-200">
                          <tr>
                            <th className="p-3 text-[10px] font-black uppercase text-slate-500 tracking-wider">Tipo da Sanção / Infração</th>
                            <th className="p-3 text-[10px] font-black uppercase text-slate-500 tracking-wider">Órgão Sancionador</th>
                            <th className="p-3 text-[10px] font-black uppercase text-slate-500 tracking-wider text-center">Período</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 bg-white">
                          {dadosCompliance.detalhes.map((sancao: any, idx: number) => (
                            <tr key={idx} className="hover:bg-red-50/30 transition-colors">
                              <td className="p-3 text-[11px] font-bold text-red-700 uppercase" title={sancao.fundamentacao}>{sancao.tipo}</td>
                              <td className="p-3 text-[11px] font-bold text-slate-600 uppercase">{sancao.orgao}</td>
                              <td className="p-3 text-[10px] font-mono font-black text-slate-500 text-center">{sancao.data_inicio} até {sancao.data_fim}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ) : (
                      <div className="p-6 text-center text-slate-500 font-medium text-xs bg-white">
                        Não constam registros nos cadastros de Empresas Inidôneas (CEIS), Punidas (CNEP) ou Impedidas (CEPIM) até o momento da consulta.
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="p-4 text-slate-400 font-bold italic text-xs">
                    Aguardando cruzamento de dados federais e listas sujas...
                  </div>
                )}
              </section>
