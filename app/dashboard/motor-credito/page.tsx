...
export default function MotorCreditoPage() {
  ...
  // Fila simulada
  const [filaMock] = useState([
    { id: "1", razao: "MADEIRAS CURITIBA LTDA", cnpj: "12.345.678/0001-99", status: "robo_processando" },
    { id: "2", razao: "CALCADOS FRANCA S/A", cnpj: "98.765.432/0001-11", status: "em_revisao_humana" },
  ]);

  const handleBuscarPorCnpj = async (e: React.FormEvent) => {
    e.preventDefault();
    
    // Limpa pontos, barras e traços para mandar o número puro para a sua API
    ...
    setLoading(true);
    try {
      // Bate na sua API passando o CNPJ limpo no formato que o promptUsuario espera
  ...
  // Máscara visual enquanto o usuário digita
      ...
      <div className="max-w-[1700px] mx-auto space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm relative overflow-hidden group">
          ...
          {!empresaSelecionada ? (
            ...
            /* LOCK DA EMPRESA + CONTAINER DO COFRE DE UPLOAD */
            <div className="space-y-4">
              ...
              {/* Área de drop de documentos vinculada */}
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
            <span className="font-black text-slate-700 uppercase tracking-widest text-[11px] flex items-center gap-2">
              📊 Status do Fluxo da Equipe Comercial
            </span>
          </div>
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-xs">
                {filaMock.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-3.5 font-black text-slate-900 uppercase truncate max-w-[300px]">{item.razao}</td>
                    <td className="p-3.5 font-mono font-bold text-slate-600">{item.cnpj}</td>
                    <td className="p-3.5">
