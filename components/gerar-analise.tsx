...
export default function GerarAnalise({ analise }: { analise: any }) {
  ...
  // Formatação segura: garante que só tente formatar números válidos
  ...
  const gerarRelatorioHTML = async () => {
    ...
    try {
      ...
      const renderizarCabecalhoEmpresas = () => {
        ...
        return `
          <div>
            <h1 style="font-size: 2.2rem; margin-bottom: 0.2rem;">${analise.razao_social || 'EMPRESA NÃO INFORMADA'}</h1>
            <div class="meta" style="font-family: monospace; font-size: 1rem;">CNPJ: ${analise.cnpj || '-'}</div>
          </div>
        `;
      };

      // ==========================================
      // TABELAS E LINHAS HTML
      // ==========================================
      let totalLimites = 0;
      ...
      // BUSCA DE IMAGENS R2
      // ==========================================
      let prefixoPasta = `clientes/${analise.id}/`; 
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.25rem; margin-bottom: 2.5rem; width: 100%;">
            ${listaFotosValidas.map(url => `
                <div class="card hover-card" style="padding: 0.5rem; display: flex; justify-content: center; align-items: center; background: #f8fafc; overflow: hidden;">
                    <img src="${url}" style="width: 100%; min-width: 100%; max-width: 100%; height: 250px; object-fit: cover; border-radius: 0.5rem;" alt="Evidência">
                </div>
            `).join("")}
        </div>
        ` : '';
      ...
      <div class="container">
          <div class="header">
              <div style="flex-grow: 1;">
                  ...
                  <div class="meta" style="margin-top: 1.5rem; padding-top: 1.25rem; border-top: 1px solid rgba(255,255,255,0.2); font-size: 0.95rem;">
                      ...
                      <strong style="color:#fff;">Analista:</strong> ${analise.analista || '-'} &nbsp;|&nbsp; 
          ...
          <div class="parecer-wrapper" style="margin-top: 3.5rem;">
              ...
              <div class="parecer-footer">
                Relatório Oficial validado digitalmente por: <strong style="color: var(--blue-dark); text-transform: uppercase;">${analise.analista || 'ANALISTA DE CRÉDITO RESPONSÁVEL'}</strong>
              </div>
          </div>
